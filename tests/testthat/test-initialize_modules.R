...
test_that("`initialize_fims()` supports GMRF recruitment process distributions", {
  n_years <- get_n_years(data)
  precision_matrix_vector <- as.vector(diag(1, n_years))

  process_template <- default_parameters |>
    dplyr::filter(
      module_name == "Recruitment",
  ...
  # Replicate one template row per precision-matrix entry.
  gmrf_process_parameters <- process_template[rep(1, length(precision_matrix_vector)), ] |>
    dplyr::mutate(
      distribution = "GMRF",
      ...
      # Precision-matrix entries are not indexed by model time here.
      time = NA_integer_
    )

  parameters_gmrf <- default_parameters |>
