  ...
  void SetupRandomEffects() {
    for (density_components_iterator it = this->density_components.begin();
      ...
      if (d->input_type == "random_effects") {
        ...
        // GMRF: wire up the precision-matrix builder (sparse-Q interface).
        //
        // Expected key layout for GMRF random effects (by convention):
        //   key[0] -> random effects vector id (already linked above)
        //   key[1] -> expected values vector id (optional; already linked above)
        //   key[2] -> n_time (stored as scalar in variable_map vector[0])
        //   key[3] -> n_variables (stored as scalar in variable_map vector[0])
        //   key[4] -> ram_type (int vector)
        //   key[5] -> ram_from (int vector, 1-based indices)
        //   key[6] -> ram_to (int vector, 1-based indices)
        //   key[7] -> ram_beta_index (int vector, 1-based; 0 means "use ram_start")
        //   key[8] -> ram_start (Type vector)
        //   key[9] -> beta_z (Type vector)
        //
        // If your keys are arranged differently, adjust the indices below.
        {
          ...
          if (gmrf) {
            ...
            auto get_vec = [&](size_t key_index, const char* what) -> fims::Vector<Type>* {
              variable_map_iterator it2 = this->variable_map.find(d->key[key_index]);
              if (it2 == this->variable_map.end()) {
                throw std::invalid_argument(
                    std::string("SetupRandomEffects: GMRF ") + what +
                    " key not found in variable_map for distribution " +
                    std::to_string(d->id) + ".");
              }
              if ((*it2).second == nullptr) {
                throw std::invalid_argument(
                    std::string("SetupRandomEffects: GMRF ") + what +
                    " pointer is null for distribution " +
                    std::to_string(d->id) + ".");
              }
              return (*it2).second;
            };
